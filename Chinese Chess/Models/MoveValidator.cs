...
namespace Chinese_Chess.Models
{
    public static class MoveValidator
    {
        ...
        public static bool IsValidMove(BoardState boardState, Move move)
        {
            ...
            if (!IsRulesValid(boardState, move)) return false;
        ...
        public static bool IsInCheck(BoardState boardState, PieceColor color)
        {
            ...
            foreach (var piece in boardState.Pieces.Where(p => p.Color != color && p.IsAlive))
            {
                ...
                var attackMove = new Move(piece, piece.X, piece.Y, general.X, general.Y);

                if (IsRulesValid(boardState, attackMove))
                {
                    return true;
                }
            }
        ...
        // ---------------------------------------------------------------------------------
        // Hàm kiểm tra luật đi cơ bản (Tách ra để tái sử dụng, tránh đệ quy)
        private static bool IsRulesValid(BoardState boardState, Move move)
        {
            ...
            {
                case PieceType.General: return IsValidGeneralMove(boardState, move);
                case PieceType.Advisor: return IsValidAdvisorMove(boardState, move);
                case PieceType.Elephant: return IsValidElephantMove(boardState, move);
                case PieceType.Horse: return IsValidHorseMove(boardState, move);
                case PieceType.Rook: return IsValidRookMove(boardState, move);
                case PieceType.Cannon: return IsValidCannonMove(boardState, move);
                case PieceType.Soldier: return IsValidSoldierMove(boardState, move);
            }
            return false;
        }
        ...
        {
            return CountPiecesInPath(boardState, move) == 0;
        }
        ...
        private static bool IsGeneralsFacing(BoardState board)
        {
        ...
        private static bool IsValidAdvisorMove(BoardState boardState, Move move)
        {
            ...
            return (Math.Abs(move.ToX - move.FromX) == 1 && Math.Abs(move.ToY - move.FromY) == 1);
        }
        ...
        private static bool IsValidCannonMove(BoardState boardState, Move move)
        {
            ...
            if (enemy == null)
            {
                if (!IsPathClear(boardState, move)) return false;
            }
            else
            {
                if (CountPiecesInPath(boardState, move) != 1) return false;
            }
            return true;
        }
